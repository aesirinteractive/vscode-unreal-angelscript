...
// Aesir Mod: add support for FieldNotifies
            if(dbprop.macroSpecifiers.has("FieldNotify"))
            {
                let notifyChangedFunc = AddDBMethod(scope, "Notify" + dbprop.name + "Changed");
                notifyChangedFunc.returnType = "void";
                notifyChangedFunc.documentation = "Broadcast the field as changed. Does not modify the value of the variable.";

                let notifySetFunc = AddDBMethod(scope, "NotifySet" + dbprop.name);
                notifySetFunc.returnType = "bool";
                notifySetFunc.isAutoGenerated = true;
                notifySetFunc.documentation = "Compare NewValue to the current value and set and broadcast the field if NewValue is different from the current value.\n" +
                    "Returns true if the field was changed and broadcast, false otherwise.";

                let notifySetArg = new typedb.DBArg();
                notifySetArg.name = "NewValue";
                notifySetArg.typename = dbprop.typename;
                notifySetFunc.args.push(notifySetArg);

                if (scope.dbtype)
                {
                    scope.dbtype.addSymbol(notifyChangedFunc);
                    scope.dbtype.addSymbol(notifySetFunc);
                }
            }
        }
// END Aesir Mod: add support for FieldNotifies
...
// Aesir Mod: add support for FieldNotifies
                if(dbfunc.macroSpecifiers.has("FieldNotify"))
                {
                    dbfunc.isFieldNotify = true;

                    let notifyFunc = AddDBMethod(scope, "Notify" + dbfunc.name + "Changed");
                    notifyFunc.returnType = "void";
                    notifyFunc.isAutoGenerated = true;
                    notifyFunc.documentation = "Broadcast the function's field as changed.";
                    if(scope.parentscope.dbtype){
                        scope.parentscope.dbtype.addSymbol(notifyFunc);
                    }
                }
// END Aesir Mod: add support for FieldNotifies
    ...
    switch (node.type)
    {
        ...
        case node_types.ClassDefinition:
        {
            ...
            for (let basetype of basetypes)
            {
                ...
                if (baseDBType)
                {
                    AddIdentifierSymbol(scope, statement, basetype, ASSymbolType.Typename, null, basetype.value);
        ...
        case node_types.InterfaceDefinition:
        {
            ...
            for (let iface of superinterfaces)
            {
                ...
                if (ifaceDBType)
                {
                    AddIdentifierSymbol(scope, statement, iface, ASSymbolType.Typename, null, iface.value);
